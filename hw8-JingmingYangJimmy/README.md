...
When a news story is selected, the user should be able brought to another screen using _a nested Stack Navigator_. This screen should show the author, posting date, and body paragraphs fetched from `https://cs571.org/api/f23/hw8/article?id=ARTICLE_ID`. Be sure that only the "Article" header bar is shown. We will add the URL in Step 5.
...
Stacks. The articles are like card to stack on the screen
...
I think is this one, since I have pressable. I can press it on my phone.

3. What other gesture(s) could be added to Badger News? What additional functionality would these gesture(s) support?

Gesture can be rotating phone. When I rotate the phone, it means that I want to go back to the homescreen

4. Is there opportunity to add any microinteraction(s)? If so, what would be the trigger-action pair(s)?

When you click preferences, the icon will change color, indicating that you are successful on the preferences tab
